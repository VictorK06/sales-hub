...
export const Route = createFileRoute("/sales")({
  ...
  head: () => ({
    meta: [
      { title: "Sales — Vendora" },
      { name: "description", content: "Register sales and review history." },
    ],
  }),
});
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="font-serif text-4xl tracking-tight">Sales</h1>
          <p className="text-muted-foreground mt-1">Every transaction, in chronological order.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            ...
            >
              <Plus className="h-4 w-4 mr-1" /> Register sale
            </Button>
          </DialogTrigger>
        ...
        {sales.isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Add products in <Link to="/products" className="underline">Products</Link>, then register your first sale.
            </p>
          </div>
        ) : (
              ...
              {sales.data.map((s) => (
                <tr key={s.id} className="border-t border-border hover:bg-accent/30 transition">
                  <td className="px-5 py-3 text-muted-foreground">
                    {format(new Date(s.sold_at), "MMM d, HH:mm")}
                  </td>
                  <td className="px-5 py-3 font-medium">
                    {s.product?.name ?? <span className="italic text-muted-foreground">deleted</span>}
                  </td>
                  ...
                  <td className="px-5 py-3 text-right">
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        if (confirm("Remove this sale? Stock will NOT be restored automatically.")) del.mutate(s.id);
                      }}
                    >
...
function SaleDialog({ onClose }: { onClose: () => void }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (selected && parsed.quantity > selected.stock) {
        throw new Error(`Only ${selected.stock} in stock`);
      }
  ...
  return (
    <DialogContent>
      <DialogHeader>
        <DialogTitle className="font-serif text-2xl">Register sale</DialogTitle>
      </DialogHeader>
